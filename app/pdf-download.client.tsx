...
function QuestionnairePdf({ appUrl, title, date, questions }: { appUrl: string; title: string; date: string; questions: { question: string; answer: string; confidence: number }[] }) {
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
...
export function useQuestionnairePdf(appUrl: string, title: string, date: string, questions: { question: string; answer: string; confidence: number }[]) {
  const doc = useMemo(() => <QuestionnairePdf appUrl={appUrl} title={title} date={date} questions={questions} />, [appUrl, title, date, questions]);
  const [url, setUrl] = useState<string | null>(null);
  useEffect(() => {
    let cancelled = false;
