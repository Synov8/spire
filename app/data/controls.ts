/**
 * Canonical control catalogue for Spire's compliance audit.
 *
 * Single source of truth for:
 *   - `db/seed.ts`               — DB upsert into the `control` table on seed
 *   - `scripts/self-audit.ts`    — CLI agent prompt construction (no DB)
 *   - `app/agents/compliance-runner.ts` & `routes/api.audit.stream.ts`
 *                                — these read from the DB at runtime; this file
 *                                  is what populates the DB on seed.
 *
 * Two frameworks, 56 SOC 2 controls + 10 EU AI Act articles — matched to the
 * PASS/FAIL counts declared on `app/data/spire-self-audit.ts`, `features.tsx`,
 * and `trust-center.tsx`. DO NOT change the totals without updating those files
 * in lockstep.
 *
 * Each entry mirrors the `control` Drizzle row shape:
 *   { id, framework, category, controlId, title, description }
 */

export type ControlSeed = {
  /** URL-safe row id. Matches Drizzle `control.id` (dot → dash). */
  id: string;
  /** "soc2" or "ai-act". */
  framework: "soc2" | "ai-act";
  /** Top-level category (e.g. "CC6", "A1", "PI1", "AI"). */
  category: string;
  /** Human-readable control number (e.g. "CC6.1", "AI-4"). */
  controlId: string;
  /** One-line title. */
  title: string;
  /** Long-form description / verification guidance. */
  description: string;
};

const RAW: ReadonlyArray<
  [ControlSeed["framework"], ControlSeed["category"], ControlSeed["controlId"], ControlSeed["title"], ControlSeed["description"]]
> = [
  // ── SOC 2 ─────────────────────────────────────────────────────────────────
...
/**
 * Frozen list — exported as the single source of truth.
 * `id` derived from `controlId` by replacing dots with dashes (matches DB
 * `control.id` convention so seed upsert is idempotent on re-seed).
 */
export const CONTROLS: ReadonlyArray<ControlSeed> = Object.freeze(
  RAW.map(([framework, category, controlId, title, description]) => ({
...
/** Convenience filters for prompts. */
export const SOC2_CONTROLS: ReadonlyArray<ControlSeed> = Object.freeze(
  CONTROLS.filter((c) => c.framework === "soc2"),
);
export const AI_ACT_CONTROLS: ReadonlyArray<ControlSeed> = Object.freeze(
  CONTROLS.filter((c) => c.framework === "ai-act"),
);

/**
 * Render a controls block for an LLM prompt section — e.g.
 *   `${SOC2_CONTROLS.map(renderPromptLine).join("\n")}`
 * keeps the same human-friendly shape the webapp agent rig expects.
 */
export function renderPromptLine(c: ControlSeed): string {
  return `${c.controlId} (${c.category}): ${c.title} — ${c.description}`;
}
